...
export default function TrustedByBusinesses() {
  return (
    <AnimatedSection className="border-b-4 border-t-red py-16 min-h-[30rem] md:py-28 bg-obsidian text-alabaster">
      <div className="flex flex-col items-center px-4 md:px-8 lg:px-12 max-w-4xl mx-auto text-center">
        ...
        <div className="flex flex-wrap justify-center gap-x-8 gap-y-4">
          {TRUSTED_BUSINESSES.map((business) => (
            <span
              key={business}
              className="text-xs uppercase tracking-wider text-alabaster hover:text-gold transition-colors cursor-default"
            >
              {business}
            </span>
          ))}
